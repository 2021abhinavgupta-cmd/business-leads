...
class AIAuditor:
    ...
    def _build_prompt(
        ...
        return (
            ...
            "CRITICAL INSTRUCTION: NUMBERS MAKE THIS LAND. People respond to numbers, not vague claims, so use one directly in the subject line itself whenever you have one, not only in the body. Every number you use must be either their own real measured number from FLAWS DETECTED above (a load time in seconds, a score out of 100, a count of broken links, a star rating, a review count), or this one real, published statistic, quoted exactly: 'Google's own research found that 53% of mobile visitors abandon a page that takes longer than 3 seconds to load.' Use that statistic only when your strongest flaw is genuinely about a slow page or slow load time, paired with their own real load time number, for example 'Your homepage takes 6.2 seconds to load on mobile. Google's own research shows over half of mobile visitors leave after 3.' You may turn that statistic into a customer style number ONLY as a clearly hypothetical, round number example, phrased so it obviously is not a claim about their real traffic, for example 'Even if just 100 people find your site this month, that means 50 or more of them may be gone before the page even finishes loading.' Always use an obviously round, clearly hypothetical visitor count in that example (say 50, 100, or 200, introduced with 'even if' or 'say'), and always base the drop off share on the real statistic above, never a share you invent yourself. NEVER state a specific number of customers, leads, calls, or an amount of revenue as if it were a measured fact about THIS business, for example 'you are losing 78 customers a month' or 'you lost 12 leads last week'. We do not have real traffic data for them, that kind of claim is exactly what they can disprove in one look, and it makes the rest of the email look fake too. The hypothetical example above is the only way a customer or lead count may appear.\n"
